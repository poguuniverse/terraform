...
resource "aws_lambda_function" "ProcessClaims" {
...
resource "aws_apigatewayv2_api" "upload_images_gw" {
...
resource "aws_apigatewayv2_stage" "upload_images_gw" {
  api_id          = aws_apigatewayv2_api.upload_images_gw.id
  auto_deploy     = true
  name            = "default"
  stage_variables = {}
  tags            = {}
...
resource "aws_apigatewayv2_integration" "upload_images_gw" {
  api_id = aws_apigatewayv2_api.upload_images_gw.id
  connection_type        = "INTERNET"
  integration_method     = "POST"
  integration_type       = "AWS_PROXY"
  integration_uri        = aws_lambda_function.upload_images_lambda.arn #"arn:aws:lambda:us-east-2:767925479794:function:my-lambda-function"
...
resource "aws_apigatewayv2_route" "upload_images_gw" {
  api_id = aws_apigatewayv2_api.upload_images_gw.id
  route_key            = "ANY /{path+}"
  target               = "integrations/${aws_apigatewayv2_integration.upload_images_gw.id}"
